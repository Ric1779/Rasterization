...
int main(int argc, char **argv)
{
    ...
    // Writing the header for a PPM file in ASCII format (P3)
    ofs << "P3\n" << imageWidth << " " << imageHeight << "\n255\n";

    // Write each pixel's RGB value in integer form to the output file
    for (int i = 0; i < imageHeight; ++i) {
        for (int j = 0; j < imageWidth; ++j) {
            // Calculate the starting index of the pixel in the buffer
            int index = (i * imageWidth + j);
            // Write the RGB values of the pixel as integers
            ofs << static_cast<int>(frameBuffer[index].x) << " " 
                << static_cast<int>(frameBuffer[index].y) << " " 
                << static_cast<int>(frameBuffer[index].z) << "\n";
        }
    }
